...
const Home = () => {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/locations/public`);
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/vehicles/public/vehicles`);
              ...
              {vehicles.slice(0, 6).map((vehicle, index) => (
                ...
                >
                  <div className={styles.imageWrapper}>
                    <img
                      src={vehicle.vehicleImage ? `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000'}/uploads/vehicles/${vehicle.vehicleImage}` : carImg}
