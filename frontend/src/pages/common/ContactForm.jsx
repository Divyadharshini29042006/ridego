...
export default function ContactForm() {
  ...
  const fetchCities = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/contact/cities`);
  ...
  const fetchSubLocations = async (city) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/contact/sublocations/${city}`);
      const data = await response.json();
